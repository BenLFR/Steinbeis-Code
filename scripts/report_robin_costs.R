...
detect_repo_root <- function() {
  env_root <- Sys.getenv("STEINBEIS_REPO_ROOT", unset = NA_character_)
  ...
  file_arg <- "--file="
  script_matches <- grep(file_arg, cmd_args, fixed = TRUE)
  if (length(script_matches) > 0) {
    script_path <- sub(file_arg, "", cmd_args[script_matches[1]])
  } else if (!is.null(sys.frames()[[1]]$ofile)) {
    script_path <- sys.frames()[[1]]$ofile
  }

  start_dir <- if (!is.null(script_path)) dirname(script_path) else getwd()
