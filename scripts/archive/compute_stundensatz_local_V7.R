...
# --- 12. HEU daily rate + contrôle SANS SUR-COMPTAGE ------------------------
...
heu_base <- mf |>
  ...
  summarise(
    months_employed = sum(employed_flag),
    wt_factor_avg   = if_else(sum(employed_flag) > 0, mean(replace_na(fte, 0)[employed_flag]), 0),
    perso_costs_RP  = sum(replace_na(gesamtkosten, 0)),
    .groups = "drop"
  ) |>
  mutate(
...
# Heures WP totales vs HEU par personne×mois (période RP)
wp_share <- cost_by_pr |>
  filter(month >= floor_date(rp_start, "month"), month <= floor_date(rp_end, "month")) |>
  group_by(du_id, month) |>
  ...
  mutate(
    hours_heu = coalesce(hours_heu, 0),
    hours_all = coalesce(hours_all, 0),
    heu_share = if_else(hours_all > 0, pmin(1, hours_heu / hours_all), 0)
  )

# Définition mf_uniq (heures réellement travaillées par du_id×mois, sans double comptage)
mf_uniq <- wt_full |>
  mutate(month = as.Date(month)) |>
  transmute(du_id, month, hours_worked = sec_any / 3600)

# Rattacher les heures réellement travaillées (non dupliquées)
wp_share2 <- wp_share |>
...
# Jours HEU déclarés sans double comptage
declared_heu_days <- wp_share2 |>
  group_by(du_id) |>
  summarise(declared_day_equiv = sum(hours_declared_heu_nodup / 8, na.rm = TRUE), .groups = "drop")
...
heu_control <- heu_base |>
  ...
  mutate(
    declared_day_equiv = replace_na(declared_day_equiv, 0),
