...
d_pr   <- read_db_ben("d_project.csv")     |> clean_names()   # <- has dpr_id, dpr_title, lpc_id
...
# Choose how to derive the window
period_mode <- "datev"   # "datev" | "union" | "manual"
...
derive_months <- function(mode=c("datev","union","manual")){
  ...
  if (mode == "manual") {
    ...
    r <- rng_from(payroll_linked, "month")
    start <- floor_date(r[1], "month"); end <- floor_date(r[2], "month")
  } else { # union of available sources
    r1 <- rng_from(payroll_linked, "month")
    r2 <- rng_from(fte_month,      "month")
    r3 <- rng_from(wt_full,        "month")
...
period_months <- derive_months(period_mode)
