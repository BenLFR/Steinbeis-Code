# compute_stundensatz_local.R – rev. 31-Jul-2025 (HEU fix)
# -------------------------------------------------------------
library(tidyverse)
library(lubridate)
library(janitor)
...
# --- 6. Map WP to cost-centers & compute work-seconds -----------------------
...
wt_by_entity <- d2wp_all %>%
  ...
  summarise(sec_entity = sum(work_secs, na.rm=TRUE), .groups="drop")
...
# --- 8. Build enriched master_personnes ------------------------------------
...
all_links <- master_personnes %>%
  ...
  bind_rows(
    multi_cc_detail %>%
      ...
      distinct() %>%
      left_join(d_user %>% select(du_id, du_login, du_name, du_surname, du_inactive),
                by = "du_id") %>%
...
# (si besoin d'un cost_by_pr_recent ailleurs)
# cost_by_pr_recent <- cost_by_pr %>% filter(month >= min_month, month <= max_month)
...
cost_by_pr <- cost_by_pr_raw %>%
  ...
  select(
    du_id, entity_code, pers_surname, pers_given, month,
    project_id = dpr_id, project, workpackage = dwp_id, wp_title,
    hours = hours_wp, cost = cost_wp, pct_time
  ) %>%
  arrange(entity_code, project, wp_title, du_id, month)

cat("✔️  cost_by_pr construit :", nrow(cost_by_pr), "lignes\n")

# ===================== 10. HEU daily rate (classe Date FIX) =================
round_half <- function(x) round(x * 2) / 2

# Définir la RP (adapter si besoin)
rp_start <- as.Date("2024-01-01")
rp_end   <- as.Date("2024-12-31")

# Convertir POSIXct -> Date pour l'alignment
mf <- master_final %>% mutate(month_date = as.Date(month))

# Mois de la RP (classe Date)
rp_months <- seq(as.Date(cut(rp_start, "month")),
                 as.Date(cut(rp_end,   "month")),
                 by = "month")

# Base HEU (compte un mois employé si FTE>0 OU coût>0)
heu_base <- mf %>%
  filter(month_date %in% rp_months) %>%
  mutate(employed_flag = (replace_na(fte,0) > 0) | (replace_na(gesamtkosten,0) > 0)) %>%
  group_by(du_id, du_name, du_surname) %>%
  summarise(
    months_employed = sum(employed_flag, na.rm = TRUE),
    wt_factor_avg   = ifelse(sum(employed_flag) > 0,
                             mean(replace_na(fte,0)[employed_flag], na.rm = TRUE), 0),
    perso_costs_RP  = sum(replace_na(gesamtkosten,0), na.rm = TRUE),
    .groups = "drop"
  )

heu_rates <- heu_base %>%
  mutate(
    day_equiv_raw  = (215/12) * months_employed * wt_factor_avg,
    day_equiv_max  = round_half(day_equiv_raw),
    heu_daily_rate = ifelse(day_equiv_max > 0, perso_costs_RP / day_equiv_max, NA_real_)
  ) %>%
  arrange(du_id)
...
heu_control <- mf %>%
  filter(month_date %in% rp_months) %>%
  group_by(du_id) %>%
  summarise(declared_day_equiv = sum(replace_na(hours_worked,0)/8, na.rm = TRUE),
            .groups="drop") %>%
  right_join(heu_rates %>% select(du_id, day_equiv_max), by = "du_id") %>%
  mutate(over_cap = declared_day_equiv > day_equiv_max)

readr::write_csv(heu_rates,   file.path(dir_db, "heu_daily_rate_by_person.csv"), na = "")
readr::write_csv(heu_control, file.path(dir_db, "heu_daily_rate_control.csv"),    na = "")
cat("✅ HEU OK : ", nrow(heu_rates), " personnes.\n")

# ============== 11. Export « photo + totaux » (WP 4034, projet 645) =========
library(readr)
